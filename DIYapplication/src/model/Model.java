...
public class Model {
	...
	public void loadProjects(final File theProjectsFile) throws IOException {
		...
		for (int i = 0; i < projectsSize; i++) {
			...
			Project project = new Project(projectName, projectDurationDay, projectCost, materials,
					Energy.getEnumVal(energyEff), projectNotes);
	...
	public void saveProjects(final File theProjectsFile) throws IOException {
		...
		for (int i = 0; i < myProjects.getProjectList().size(); i++) {
			...
			fileWriter
					.write(myFileEnergyEffHeader + myProjects.getProjectList().get(i).getMyEnergy().getValue() + "\n");
			...
			fileWriter.write(myFileMaterialsSizeHeader
					+ myProjects.getProjectList().get(i).getMyMaterials().getMaterialMap().size() + "\n");
			for (Map.Entry<String, Double> entry : myProjects.getProjectList().get(i).getMyMaterials().getMaterialMap()
					.entrySet()) {
